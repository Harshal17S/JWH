...
async function match() {
   ...
   // debugger;
const mtcnnResults = await faceapi.ssdMobilenetv1(document.getElementById('viewer'))
...
const force = (document.getElementById('voterId').value).trim()
try{
  if (force =="") throw new error(e)
}
catch(e){
  var errorMessage = document.getElementById('error')
  errorMessage.textContent = 'Voter Id field cannot be left blank'
  setTimeout(() => { window.location.href = 'http://localhost:3000/login.html' }, 3000);
}
 ...
 try{
  ...
  var errorMessage = document.getElementById('error')
  errorMessage.textContent = 'Face not detected or voter Id not found... try again'
  setTimeout(() => { window.location.href = 'http://localhost:3000/login.html' }, 3000);
